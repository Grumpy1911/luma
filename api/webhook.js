...
// De project-URL is geen geheim, die mag hier blijven staan.
const SUPABASE_URL = 'https://pgexambwfqchlkhlbywo.supabase.co';

// De sleutel wél. Deze functie schrijft bestellingen weg en heeft daarvoor de
// service_role key nodig, want die omzeilt Row Level Security. Er staat
// bewust géén terugval op de anon key: die terugval zorgde ervoor dat een
// ontbrekende sleutel niet opviel — de bestelling verdween dan gewoon.
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (payment.status === 'paid') {
            ...
            if (!sbKey) {
                console.error('SUPABASE_SERVICE_KEY ontbreekt — bestelling niet opgeslagen:', payment.id);
...

            {
                ...
                if (!sbRes.ok) {
                    // 500 teruggeven in plaats van 200: Mollie probeert het dan
                    // opnieuw. Vroeger werd deze fout opgeslokt en dacht Mollie
                    // dat alles goed was — een betaalde bestelling was dan weg.
                    console.error('Supabase opslaan mislukt:', sbRes.status, await sbRes.text());
                    return res.status(500).end();
                }
